...
Inventory writes run in serializable PostgreSQL transactions. The application checks the authenticated store and Owner role, conditionally advances the balance version, appends exactly one movement, writes an audit event, and stores the completed response under a store-scoped idempotency key. Serialization conflicts retry up to three times and otherwise return refresh-and-retry guidance.
