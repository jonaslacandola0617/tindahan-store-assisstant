...
export default async function InvitePage({ params }: { params: Promise<{ token: string }> }) {
  ...
  return <main className="standalone"><section className="card invite-page invite-card"><div><h1>Join {invitation.store.name}</h1><p className="text-muted" style={{ marginTop: "var(--space-2)" }}>You were invited as staff. Your access stays separate from the owner’s account.</p></div><div className="invite-store"><strong>{invitation.email}</strong><p className="text-sm text-muted">Staff account</p></div><InviteAcceptForm token={token} signedIn={Boolean(session?.user.id)}/>{!session?.user.id && <p className="text-sm text-muted" style={{ textAlign: "center" }}>Already have an account? <Link href={`/sign-in?callbackUrl=${encodeURIComponent(`/invite/${token}`)}`} style={{ color: "var(--color-brand-primary)", fontWeight: "var(--weight-semibold)" }}>Sign in with the invited email</Link></p>}</section></main>;
}
