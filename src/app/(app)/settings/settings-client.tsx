...
type SettingsData = {
  ...
  subscription: { plan: "PILOT" | "TRIAL" | "STANDARD"; status: "TRIALING" | "ACTIVE" | "GRACE" | "RESTRICTED" | "CANCELED"; trialEndsAt: string | null; currentPeriodEndsAt: string | null; graceEndsAt: string | null; onlineBillingAvailable: boolean };
...
type BillingData = { statements: { id: string; statementNumber: string; total: string; currency: string; paymentStatus: string; createdAt: string }[] };
...
export function SettingsClient({ initial, initialBilling, locale }: { initial: SettingsData; initialBilling: BillingData; locale: "EN" | "FIL" }) {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try { const response = await fetch("/api/settings/staff", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ email: form.get("email") }) }); const result = await responseMessage(response, t.genericError) as { id: string; email: string; expiresAt: string; inviteUrl: string; emailStatus: "SENT" | "FAILED" }; setData(current => ({ ...current, invitations: [{ id: result.id, email: result.email, expiresAt: result.expiresAt, createdAt: new Date().toISOString(), emailStatus: result.emailStatus }, ...current.invitations] })); setInviteUrl(result.inviteUrl); setInviteLinkId(result.id); formElement.reset(); setToast(t.inviteCreated); }
  ...
  async function resend(id: string) { setPending(true); setError(""); try { const response = await fetch(`/api/settings/staff/invitations/${id}/resend`, { method: "POST" }); const result = await responseMessage(response, t.genericError) as { id: string; email: string; expiresAt: string; inviteUrl: string; emailStatus: "SENT" | "FAILED" }; setData(current => ({ ...current, invitations: [{ id: result.id, email: result.email, expiresAt: result.expiresAt, createdAt: new Date().toISOString(), emailStatus: result.emailStatus }, ...current.invitations.filter(item => item.id !== id)] })); setInviteUrl(result.inviteUrl); setInviteLinkId(result.id); setToast(t.inviteCreated); } catch (cause) { setError(cause instanceof Error ? cause.message : t.genericError); } finally { setPending(false); } }
  async function startPlan() { setPending(true); setError(""); try { const response = await fetch("/api/billing/checkout", { method: "POST" }); const result = await responseMessage(response, t.genericError) as { checkoutUrl: string }; window.location.assign(result.checkoutUrl); } catch (cause) { setError(cause instanceof Error ? cause.message : t.genericError); setPending(false); } }
  async function cancelPlan() { setPending(true); setError(""); try { const response = await fetch("/api/billing/cancel", { method: "POST" }); const result = await responseMessage(response, t.genericError) as { message: string }; setToast(result.message); router.refresh(); } catch (cause) { setError(cause instanceof Error ? cause.message : t.genericError); } finally { setPending(false); } }
  ...
  return <>
    ...
    {owner && <>
      <SettingsGroup title={t.team}>
        ...
        {inviteUrl && <div className="info-banner invite-link"><span className="text-sm">{inviteUrl}</span><button className="btn btn-secondary btn-sm" type="button" onClick={async () => { await navigator.clipboard.writeText(inviteUrl); setToast(t.copied); }}>{t.copyLink}</button></div>}
