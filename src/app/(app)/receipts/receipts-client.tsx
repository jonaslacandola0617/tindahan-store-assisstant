...
export function ReceiptsClient({locale,initial,showcase=false}:{locale:"EN"|"FIL";initial:Results;showcase?:boolean}){
  ...
  return <>
    <div className="content-header"><div className="content-header-row"><div><h1>{t.title}</h1><p className="text-muted">{showcase?(locale==="FIL"?"Sample receipt activity para sa public employer demo.":"Sample receipt activity for the public employer demo."):t.subtitle}</p></div>{showcase?<span className="badge badge-info"><Icon name="info" className="icon icon-sm"/>{locale==="FIL"?"Sample receipts":"Sample receipts"}</span>:<Link className="btn btn-primary" href="/receipts/new"><Icon name="camera"/>{t.scan}</Link>}</div></div>
    ...
    {loading&&results.items.length===0?<SectionLoading message={loadingCopy(locale,"receipts")}/>:error?<div className="empty-state"><div className="empty-icon"><Icon name="alert"/></div><h3>{error}</h3><button className="btn btn-secondary" onClick={()=>load()}>{t.retry}</button></div>:results.items.length===0?<div className="empty-state"><div className="empty-icon receipt-empty-icon"><Icon name="receipt"/></div><h3>{t.empty}</h3><p className="empty-body">{showcase?(locale==="FIL"?"Walang sample receipt sa filter na ito.":"There are no sample receipts in this filter."):t.emptyBody}</p>{!showcase&&<Link className="btn btn-secondary" href="/receipts/new">{t.scan}</Link>}</div>:<div aria-busy={loading}>{results.items.map(item=>{
      const s=status(item);
