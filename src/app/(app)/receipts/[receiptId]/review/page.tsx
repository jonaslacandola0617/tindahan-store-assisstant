...
export default async function ReceiptReviewPage({params}:{params:Promise<{receiptId:string}>}){const session=await getServerSession(authOptions);if(!session?.user.id)redirect("/sign-in");const locale=(await cookies()).get("tindahan-language")?.value==="FIL"?"FIL":"EN";const receipt=await readReceipt(session.user.id,(await params).receiptId,process.env.NEXTAUTH_URL??"http://localhost:3000");return <ReceiptReviewClient locale={locale} initial={receipt}/>;}
