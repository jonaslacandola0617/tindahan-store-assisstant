...
const copy = {
  EN: {
    ...
    packageBarcode: "Use the barcode on the package", generate: "Generate barcode", tingi: "Best for tingi items", none: "No barcode", without: "Continue without one", manufacturer: "Manufacturer barcode",
  ...
  FIL: {
    ...
    packageBarcode: "Gamitin ang barcode sa pakete", generate: "Gumawa ng barcode", tingi: "Para sa tingi na produkto", none: "Walang barcode", without: "Magpatuloy nang wala nito", manufacturer: "Barcode ng gumawa",
...
export function InventoryClient({ initial, initialView, locale }: { initial: Results; initialView: "LIST" | "GRID"; locale: "EN" | "FIL" }) {
  ...
  return <>
    ...
    <div className="inventory-results-bar"><p className="text-sm text-muted" role="status" aria-live="polite">{loading ? t.loading : `${results.items.length} ${resultLabel}`}</p><div className="segmented view-toggle" role="group" aria-label={locale === "FIL" ? "Ayos ng imbentaryo" : "Inventory view"}><button className={view === "LIST" ? "active" : ""} aria-pressed={view === "LIST"} onClick={() => changeView("LIST")}><Icon name="list" className="icon icon-sm"/>{t.list}</button><button className={view === "GRID" ? "active" : ""} aria-pressed={view === "GRID"} onClick={() => changeView("GRID")}><Icon name="grid" className="icon icon-sm"/>{t.grid}</button></div></div>
    ...
    {results.nextCursor && <div className="load-more-row"><button className="btn btn-secondary" disabled={loading} onClick={() => load(query, filter, results.nextCursor!)}>{t.loadMore}</button></div>}
    {notice && <div className="toast is-visible" role="status">{notice}</div>}

    {modal && <><div className="overlay-scrim is-open" onClick={closeModal}/><div className="modal is-open" role="dialog" aria-modal="true" aria-labelledby="add-title"><div className="modal-card" ref={dialog}>
      ...
      <form onSubmit={submit} className="modal-form">
        ...
        <div className="modal-actions"><button type="button" className="btn btn-secondary" onClick={closeModal}>{t.cancel}</button><button className="btn btn-primary" type="submit">{t.save}</button></div>
      </form>
    </div></div></>}
  </>;
}
