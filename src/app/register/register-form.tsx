...
export function RegisterForm({ locale }: { locale: Locale }) {
  ...
  return <><h1 style={{ marginBottom: "var(--space-2)" }}>{text.title}</h1><p className="text-muted" style={{ marginBottom: "var(--space-6)" }}>{text.intro}</p><form className="auth-form" onSubmit={submit}>{error && <p className="form-alert" role="alert">{error}</p>}<div className="field"><label className="field-label" htmlFor="register-email">{text.email}</label><input className={`input${errors.email ? " has-error" : ""}`} id="register-email" name="email" type="email" placeholder="you@example.com" autoComplete="email" required aria-invalid={Boolean(errors.email)} aria-describedby={errors.email ? "register-email-error" : undefined} onChange={() => clear("email")}/>{errors.email && <span className="field-error" id="register-email-error">{errors.email}</span>}</div><div className="field"><label className="field-label" htmlFor="register-password">{text.password}</label><input className={`input${errors.password ? " has-error" : ""}`} id="register-password" name="password" type="password" placeholder="••••••••" autoComplete="new-password" required maxLength={128} aria-invalid={Boolean(errors.password)} aria-describedby={errors.password ? "register-password-error" : "register-password-hint"} onChange={() => clear("password")}/>{errors.password ? <span className="field-error" id="register-password-error">{errors.password}</span> : <span className="field-hint" id="register-password-hint">{text.passwordHint}</span>}</div><div className="field"><label className="field-label" htmlFor="register-confirm-password">{text.confirm}</label><input className={`input${errors.confirmPassword ? " has-error" : ""}`} id="register-confirm-password" name="confirmPassword" type="password" placeholder="••••••••" autoComplete="new-password" required maxLength={128} aria-invalid={Boolean(errors.confirmPassword)} aria-describedby={errors.confirmPassword ? "register-confirm-error" : undefined} onChange={() => clear("confirmPassword")}/>{errors.confirmPassword && <span className="field-error" id="register-confirm-error">{errors.confirmPassword}</span>}</div><button className="btn btn-primary btn-lg btn-block" type="submit">{text.continue}</button></form><p className="text-sm text-muted" style={{ textAlign: "center", marginTop: "var(--space-6)" }}>{text.existing} <Link href="/sign-in" style={{ color: "var(--color-brand-primary)", fontWeight: "var(--weight-semibold)" }}>{text.signIn}</Link></p></>;
}
