...
databaseTests("operating view PostgreSQL integration", () => {
  ...
  beforeAll(async () => {
    const db = database();
    const owner = await db.user.create({ data: { email: `phase6-owner-${suffix}@example.test` } });
    const outsider = await db.user.create({ data: { email: `phase6-outsider-${suffix}@example.test` } });
    userId = owner.id; outsiderId = outsider.id;
    const store = await db.store.create({ data: { name: `Phase 6 ${suffix}`, preference: { create: {} }, memberships: { create: { userId, role: "OWNER" } } } });
    const other = await db.store.create({ data: { name: `Other Phase 6 ${suffix}`, preference: { create: {} }, memberships: { create: { userId: outsiderId, role: "OWNER" } } } });
    ...
    await db.inventoryMovement.create({ data: { storeId, productId, actorId: userId, type: "SALE", quantityDelta: -2, previousQuantity: 4, resultingQuantity: 2, sourceType: "SALE", sourceId: sale.id, correlationId: `phase6-${suffix}` } });
