...
const updateInput = productInput.omit({ startingQuantity: true, barcodeChoice: true, manufacturerBarcode: true }).partial();
...
export async function searchProducts(userId: string, raw: { query?: string; filter?: string; sort?: string; cursor?: string; limit?: number }) {
  ...
  const records = await database().product.findMany({ where, take: limit + 1, ...(raw.cursor ? { cursor: { id: raw.cursor }, skip: 1 } : {}),
    orderBy: raw.sort === "name" ? [{ normalizedName: "asc" }, { id: "asc" }] : raw.sort === "quantity" ? [{ balance: { quantity: "asc" } }, { id: "asc" }] : [{ updatedAt: "desc" }, { id: "asc" }],
    include: { category: true, balance: true, barcodes: { where: { status: "ACTIVE" }, select: { type: true } }, supplierReferences: { include: { supplier: true }, take: 1 } } });
  let items = records.slice(0, limit).map(product => ({ ...product, sellingPrice: product.sellingPrice.toString(), latestPurchaseCost: product.latestPurchaseCost?.toString() ?? null,
  ...
  if (filter === "low") items = items.filter(item => item.stockStatus === "low_stock");
...
export async function createProduct(userId: string, raw: unknown) {
  ...
  }, { isolationLevel: "Serializable" }).catch(error => { if ((error as {code?:string}).code === "P2002") throw new InventoryError("DUPLICATE_BARCODE", "That barcode is already assigned to another product.", 409); throw error; });
}

export async function updateProduct(userId: string, productId: string, raw: unknown) {
  const { store } = await contextFor(userId, true); const input = updateInput.parse(raw); const existing = await database().product.findFirst({ where: { id: productId, storeId: store.id } });
  ...
  return database().product.update({ where: { id: productId }, data });
}
...
async function changeStock(userId: string, productId: string, raw: unknown, kind: "add"|"adjust") {
  const { store } = await contextFor(userId, true); const parsed = kind === "add" ? stockInput.parse(raw) : adjustmentInput.parse(raw); const delta = kind === "add" ? parsed.quantity : parsed.delta; const scope = kind === "add" ? "ADD_INVENTORY" : "ADJUST_INVENTORY"; const hash = requestHash(parsed);
...
export async function assignBarcode(userId: string, productId: string, raw: unknown) {
  const { store } = await contextFor(userId, true); const value = validateBarcode(z.object({ value: z.string() }).parse(raw).value);
  try { return await database().productBarcode.create({ data: { storeId: store.id, productId, assignedById: userId, value, normalizedValue: value, type: "MANUFACTURER" } }); } catch (error) { if ((error as {code?:string}).code === "P2002") throw new InventoryError("DUPLICATE_BARCODE", "That barcode is already assigned to another product.", 409); throw error; }
}

export async function generateBarcode(userId: string, productId: string, replace = false) {
  ...
  return database().$transaction(async tx => { const product = await tx.product.findFirst({ where: { id: productId, storeId: store.id, status: "ACTIVE" } }); if (!product) throw new InventoryError("NOT_FOUND", "Product not found.", 404);
    ...
    return tx.productBarcode.create({ data: { storeId: store.id, productId, assignedById: userId, value, normalizedValue: value, type: "INTERNAL", replacesId: previous?.id } });
